...
## M12 — External advanced-disease replication (Landa 2016 GSE76039) [PAPER 1, NEW 2026-05-04]

GSE76039 (Landa et al. 2016 JCI 126(3):1052–1066; PubMed 26878173) provides transcriptomic profiles of 37 advanced thyroid cancers (17 poorly-differentiated, PDTC; 20 anaplastic, ATC; no PTC controls in series; no normal in series; Affymetrix HG-U133 Plus 2.0 microarray, GPL570). Series Matrix expression values were downloaded as gcRMA log₂ from GEO (`GSE76039_series_matrix.txt.gz`); no raw CEL re-normalisation was performed by us. Probe-to-gene mapping used the GPL570 platform annotation table (NetAffx build 35; "Gene Symbol" column); per gene, the probe with the highest mean expression across the 37 samples was retained ("max-mean" collapse). Per-sample, per-gene within-cohort z-scores were computed; module scores were the across-gene mean of z-scores per sample. The TCGA-trained absolute-form classifier was **not** applied across platforms (per `v17_korean_K2_calibration` calibration memo); within-cohort z normalisation is the only operation. Module definitions: RAI_8 = {SLC5A5, TPO, TG, TSHR, PAX8, NKX2-1, FOXE1, DIO1}; THYROID_NONOVERLAP = {SLC26A4, IYD, DUOX1, DUOX2, TFF3, HHEX, GLIS3, DIO2}; TF_collapse = {FOXE1, NKX2-1, PAX8, HHEX}; STAT3_AP1_DNMT = {STAT3, FOSL1, JUNB, DNMT1, DNMT3B}; TACSTD2 single-gene z. ATC vs PDTC contrasts used Mann–Whitney U (two-sided) and Cohen's d (pooled SD); within-cohort correlations used Spearman ρ. No survival, mutation, or stage-ordinal claims were derived (mutation/survival metadata absent in GEO); the within-series ATC vs PDTC contrast is the only stage contrast available, and inter-cohort claims to TCGA-PTC are not made (cross-platform PTC reference is out of scope for this acquisition slot). All inputs, scripts, intermediate outputs, and figure source files are at `project/results/p_landa_2016/` and `project/notebooks_or_scripts/p_landa_2016_first_pass.py`; per-sample scores at `Suppl Table SX_LANDA_GSE76039.xlsx`.
