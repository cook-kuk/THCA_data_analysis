# Paper 1 — strategy memo + Hook ¶1 checklist (combined view)

**Date:** 2026-05-04
**Owner:** Seungho Cook
**Mode:** Combined read-only memo. **No new analysis. No download. No RunPod. No Paper 3/4 touch. No voice-protected prose.**
**Purpose:** Single VSCode-viewable file combining (Part A) the 8-gene-curated vs de-novo / driver-map final strategy memo, and (Part B) the Hook ¶1 preparation checklist.
**Source files (each authoritative on its own):**

- Part A is identical to `project/reports/2026_05_04_8gene_curated_vs_denovo_final_strategy.md`
- Part B is the chat-only Hook ¶1 preparation checklist, gathered here for reference

---

# PART A — 8-gene curated vs de novo / driver-map strategy — final decision memo
...
# PART B — Hook ¶1 final preparation checklist

**Mode:** Bullets only. **Do not draft Hook prose. Do not edit manuscript files.** This checklist is for the author to write Hook ¶1 by keyboard.

## B.1 Angle to choose
- **Lead frame** = transcriptional differentiation axis orthogonal to BRAF/RAS, with TROP2 vulnerability hook (per Part A §12.3, T1)
- **Avoid lead frames:** 8-gene-as-discovery, "BRAF/RAS-negative dark matter has worse outcome," HT/autoimmune (Paper 2), HLA forest (Paper 4)
- **Hook spine = 4 beats:** clinical gap → existing molecular landscape limit → orthogonal axis we define → translational consequence (TROP2)
- **Tone phrase choice (pick one for 본인 voice):** "no mechanistic compass" (dramatic) / "lacks a transcriptional axis to predict" (specific) / "without molecular guidance" (middle)

## B.2 4-sentence structure
- **S1 — Clinical gap:** DTC overall survival vs ATA 2015 recurrence heterogeneity (15–35% overall PTC; 3–5% low → 50–75% high)
- **S2 — Existing molecular landscape limit:** TCGA 2014 BRAF-like / RAS-like + Yoo 2016 TDS-16 already exist; ATA 2015 itself notes BRAF/TERT only "marginal benefit" — gap = within-tier transcriptional axis still missing
- **S3 — What we do:** define a transcriptional differentiation axis robust at the unrestricted transcriptome level, orthogonal to canonical drivers, mechanistically anchored (TF-collapse → DNMT → STAT3/AP-1)
- **S4 — Translational consequence:** independent prognostic association + tumor-population TROP2 vulnerability framing
- **Length budget:** 4 sentences total; 110–150 words; no semicolon-stacked clauses

## B.3 Safe numbers (only these — do not introduce new ones)
- `15–35% recurrence` (overall PTC, ATA 2015) — **preferred Hook stat**
- `3–5%` low-risk → `50–75%` high-risk structural recurrence (Tuttle/Alzahrani 2019)
- `>98%` 5-yr OS (only if needed for contrast)
- TCGA-THCA `n=500` (cohort)
- DM1_like vs THYROID_NONOVERLAP `r=−0.885 (n=561)` — only if Hook needs an anchor number; usually move to Results
- Pan-genome top-5000 MAD `ARI≈0.92` ≈ TIERA67 `ARI≈0.90` — usually move to Results
- DFI multivariate `HR=1.41, p=0.025` — usually move to Abstract/Results, not Hook
- **Forbidden in Hook:** `5–15%` (source unclear, removed per HOOK_STAT brief), `96%`, `94.6%`, `53/56`, `~99%`, `0.49`, `0.013`, BRS-specific numbers, venue probabilities

## B.4 Forbidden claims (per `molecular_only_lock` §5)
- `proves` / `establishes` / `all risks resolved` / `모든 risk 해소`
- `Cancer Cell-ready` / `Nat Cancer reach` / venue probability percentages
- `H&E-inferable` / `H&E predicts DM1` / `pathology-AI triage` / `morphology-derived DM1` / `WSI-validated DM1` / `tile-level DM1 inference`
- `TROP2 spatially colocalizes with DM1-high spots` / `DM1-high spots are TROP2-high`
- `RunPod G1/G2/G3` / `image-DM1 Phase B/C` (active-claim form)
- `BRAF/RAS-negative dark matter has worse outcome` (N1 NS — supp only)
- `drivers explicitly excluded by design` (audit corrected — they were in TIERA67 candidate pool, ranked low)
- `An 8-gene panel discovers...` / "novel 8-gene panel" framing (8/8 ⊂ Yoo 2016 TDS-16)
- `actionable in DM1 patients` (no functional validation)
- `demonstrates` 4-step framework (use `consistent with`)

## B.5 Citation reminders
- **Hook stat citation:** Haugen 2016 ATA Guidelines (Thyroid 26(1):1-133, doi 10.1089/thy.2015.0020, PMC4739132) — `Haugen2016ATA` in `2026_05_03_references.bib`
- **Stratified rate:** Tuttle/Alzahrani 2019 JCEM 104(9):4087-4100 — `Tuttle2019` (memory `paper1_bib_citation_audit`: was on the "unused bib" list — verify before finalising)
- **TCGA dichotomy:** TCGA Cancer Genome Atlas Research Network 2014 Cell — verify exact bibkey in `2026_05_03_references.bib`
- **Yoo 2016 TDS-16:** Yoo et al. 2016 PLOS Genet — verify bibkey
- **Landa 2016 PDTC/ATC:** Landa 2016 JCI 126(3):1052-1066 — **REQUIRED**, replaces the earlier Krishnamoorthy mis-attribution (memory `v17_landa2016_cite_save`)
- **Krishnamoorthy 2025:** if cited for ATC dedifferentiation, verify it lives in bib AND that no facts mis-attributed to it (4 facts were wrong — see memory)
- **ATA "marginal benefit" quote** (BRAF/TERT) traceable to Haugen 2016 — Discussion §3.2 already drafted in `ATA2015_cheatsheet.md` §7
- **Bib housekeeping pending:** `2026_05_04_bib_m3m4_straggler_cleanup.md` (M3 Lee2024, M4 Lim2025) + L4 (header "~22" vs actual 29) + L7 (4 unused: Tuttle2019, Yi2016, Chen2024, Kim2014) — confirm before submission, not blocking Hook draft

## B.6 After-draft audit criteria (run as a single pass on the finished Hook)
- [ ] Word count 110–150; sentence count exactly 4
- [ ] Hook stat is `15–35%` (or stratified `3–5%` → `50–75%`); **no `5–15%`**
- [ ] No forbidden phrase from §B.4 above (grep: `proves|establishes|all risks resolved|H&E|colocalizes|colocalize|dark matter|pathology-AI|RunPod|Cancer Cell|Nat Cancer|excluded by design`)
- [ ] No 8-gene panel name (`SLC5A5`, `TPO`, `DIO1`, etc.) in Hook — they belong to Methods/Results
- [ ] No HLA / Hashimoto / Graves / TLS / BCR mentions (Paper 2/4 boundary)
- [ ] No image / WSI / pathology-AI mentions (closure NO-GO)
- [ ] No venue / probability / "ready for X" wording
- [ ] Driver-orthogonality framed as orthogonality, **not** as "BRAF/RAS-negative subset has worse outcome"
- [ ] TROP2 framed as `tumor-population vulnerability` (or held until Abstract/Results, if not in Hook)
- [ ] Citations present at: ATA stat (Haugen2016ATA, optional Tuttle2019), TCGA 2014, Yoo 2016, Landa 2016 — at least one of TCGA/Yoo/Landa cited in S2
- [ ] All Hook citations exist in `2026_05_03_references.bib` (grep bibkeys before commit)
- [ ] Voice phrase aligned with `manuscript v2.5 reviewer-tone preference` (one of the 4 in HOOK_STAT §본인 voice 영역)
- [ ] Hook reads cleanly as ¶1 — no lab-jargon (`Cohen's d`, `ARI`, `padj`, `LFC`) leaks in
- [ ] No future-tense overpromises ("we will demonstrate...") — all stated as `we define`/`we show`/`supports`
- [ ] Final read-aloud: matches Discussion §3.1 framing (you'll write that next; keep tonal continuity)
- [ ] Run repo grep: `grep -nE "5-15%|5–15%|H&E-infer|colocalizes with DM1|excluded by design|Cancer Cell-ready" <Hook file>` — must return zero
- [ ] Confirm Hook does NOT touch any of the other 5 voice-protected sections (Aim ¶4, Discussion §3.1, §3.4 Limitations, Cover ¶1, Reviewer Q9) in the same edit

---

*Combined memo authored 2026-05-04 by Claude (Opus 4.7) under marathon-mode discipline. Read-only audit + design-only strategy + checklist. No new analysis, no download, no RunPod, no voice-protected prose generated, no Paper 3/4 touch, no manuscript file modified, no commit. All claims sourced from molecular_only_lock §2 envelope or pre-existing reports/memories listed in the headers.*
