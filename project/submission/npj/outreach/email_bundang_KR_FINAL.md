...
최근 TCGA-THCA 513명 갑상선 유두암 환자의 transcriptomic 재분석을 통해 **임상적으로 즉시 적용 가능한 RAI 반응성 biomarker**를 도출하였고, 본 연구를 **npj Precision Oncology** 에 제출하기 직전입니다 (manuscript v6, 모든 figure + 정직한 leak-free 재validation 완료). 한국인 cohort 에서의 cross-validation 을 통해 paper 의 임상적 신뢰도를 한 단계 높이고자, 분당서울대병원 갑상선 환자 데이터 활용에 대한 협력을 정중히 제안드립니다.

## 1. 연구 핵심 결과

| 항목 | 결과 |
|------|------|
| **8-gene RAI 반응성 패널** (NIS, TPO, TG, TSHR, PAX8, NKX2-1, FOXE1, DIO1) | 5-fold CV AUC = **0.962** (leak-free, R1-A) |
| 대비 BRAF V600E 단독 baseline | AUC = 0.849 (**ΔAUC = +0.113**) |
| Zero-overlap functional cluster (R1-B, MAPK+immune+EMT) | AUC = 0.925 (8-gene 과 0개 gene overlap; circular 아님 입증) |
| BRS framework cluster (R1-D) | AUC = 0.957, ΔAUC vs BRAF +0.111 |
| 외부 검증 (GSE76039 ATC vs PDTC, histology ground truth, n = 37) | AUC = **0.935 (95% CI 0.824-1.000)** |
| MSK-IMPACT n=231 thyroid TERT prevalence cross-cohort | PTC 63.4%, ATC 81.8% (cross-validated context) |
