...
best_auprc = m.sort_values("AUPRC", ascending=False).iloc[0]
best_late = late[late["status"].eq("prespecified_equal_weight")].sort_values("AUPRC", ascending=False).iloc[0]
best_top10 = m.sort_values("top10_precision", ascending=False).iloc[0]
strict = m[m["split_name"] == "repeated_stratified_5x5_internal"].sort_values("AUPRC", ascending=False).iloc[0]
retr = pd.read_csv(out / "retrieval_leakage_audit.tsv", sep="\t")
hla = m[m["split_name"].str.contains("hla", na=False)].sort_values("AUPRC", ascending=False)
...
print(f"best model by AUPRC: {best_auprc['feature_group']} / {best_auprc['model']} / {best_auprc['split_name']} AUPRC={best_auprc['AUPRC']:.3f} AUROC={best_auprc['AUROC']:.3f}")
...
print(f"strict no-reference result: {strict['feature_group']} / {strict['model']} AUPRC={strict['AUPRC']:.3f} AUROC={strict['AUROC']:.3f} top10={strict['top10_precision']:.3f}")
