#!/usr/bin/env python3
"""Build a flagship command center for CROSS-Neo.

This is the highest operational layer in the current stack. It converts the
flagship packet into a concrete command board with explicit go/hold rules,
refresh actions, and the next experimental branch for each lead.
...
PAGE = HUB / "cross_neo_flagship_command_center.html"
WEB_PAGE = WEB / "cross_neo_flagship_command_center.html"
OUT = MD_OUT / "flagship_command_center"
FIG = MD_OUT / "figures"
...
def safe_copy(src: Path, dst: Path) -> None:
    try:
        ...
        print(f"[flagship-command-center] skip permission-denied copy {dst}")
...
def load_inputs() -> dict[str, pd.DataFrame]:
    ...
    if data["packet"].empty:
        raise FileNotFoundError(PACKET / "flagship_execution_packet.tsv")
    return data


def build_board(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    packet = data["packet"].copy()
    packet["command_branch"] = [
        "endpoint energy first; PMF/FEP held unless endpoint is favorable",
        "endpoint energy first; PMF only if TCR-pMHC stays stable",
    ]
    packet["current_action"] = [
        "prepare WT/decoy-controlled HLA stability and binding setup",
        "prepare WT/decoy-controlled HLA stability and binding setup",
    ]
    packet["abort_condition"] = [
        "WT or decoy does not stay separated from mutant on endpoint gate",
        "pairing or TCR model becomes unstable",
    ]
    packet["next_wetlab_step"] = [
        "multimer/activation only after presentation gate passes",
        "multimer/activation only after presentation gate passes",
    ]
...
def make_figure(board: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(13.2, 6.2))
    ax.axis("off")
    boxes = [
        (0.04, 0.64, 0.20, 0.20, "Refresh stack\nconsole -> atlas -> matrix"),
        (0.28, 0.64, 0.20, 0.20, "Physics gate\nendpoint energy first"),
        (0.52, 0.64, 0.20, 0.20, "Execution packet\n72h board"),
        (0.76, 0.64, 0.20, 0.20, "Wetlab branch\nWT/decoy assay"),
        (0.28, 0.16, 0.68, 0.18, "Command rule: do not promote to PMF/FEP or activation assays unless the endpoint gate remains favorable and WT/decoy controls stay clean."),
    ]
    ...
    for start, end in [((0.24, 0.74), (0.28, 0.74)), ((0.48, 0.74), (0.52, 0.74)), ((0.72, 0.74), (0.76, 0.74))]:
        ax.annotate("", xy=end, xytext=start, arrowprops=dict(arrowstyle="->", color="#edf5ff", lw=2))
    ax.text(0.04, 0.93, "CROSS-Neo flagship command center", fontsize=18, color="#fff8e8", fontweight="bold")
    ax.text(0.04, 0.06, "Operational layer only. This page coordinates ranking, physics escalation, and assay gating; it does not prove immunogenicity.",
            fontsize=11, color="#cbd7e7")
    fig.tight_layout()
    fig.savefig(FIG / "fig_md73_flagship_command_center.png", dpi=180, bbox_inches="tight")
    fig.savefig(FIG / "fig_md73_flagship_command_center.pdf", bbox_inches="tight")
    plt.close(fig)


def write_report(board: pd.DataFrame, data: dict[str, pd.DataFrame]) -> None:
    lines = [
        "# CROSS-Neo flagship command center",
        "",
        "## Executive verdict",
        "",
        "This is the highest operational layer in the current stack. It turns the two flagship leads into a single command board with explicit branch conditions.",
        "",
        "## Key numbers",
        "",
        ...
        "## Claim boundary",
        "",
        "This command center coordinates execution and escalation. It does not prove immunogenicity, activation, killing, or clinical utility.",
        "",
    ]
    (OUT / "FLAGSHIP_COMMAND_CENTER.md").write_text("\n".join(lines) + "\n")
    (OUT / "FLAGSHIP_COMMAND_CENTER_ONE_PAGE_KR.md").write_text(
        "\n".join(
            [
                "# CROSS-Neo flagship command center 요약",
                "",
                "- 최고 운영층 command board",
                "- endpoint energy -> WT/decoy assay branching",
                "- PMF/FEP는 endpoint 통과 후만",
                "",
            ]
        )
        + "\n"
...
def write_html(board: pd.DataFrame, data: dict[str, pd.DataFrame]) -> None:
    html_text = f"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>CROSS-Neo Flagship Command Center</title>
<style>
...
.wrap{{max-width:1460px;margin:0 auto;padding:24px 30px 72px}}
...
<h1>CROSS-Neo Flagship Command Center</h1>
<p class="lead">The operational apex of the current stack. It compresses the two flagship leads into one command board with branch conditions, refresh order, and assay escalation rules.</p>
<div class="stats">
<div class="stat"><b>{len(board)}</b><span>flagships</span></div>
...
<div class="warn">This command center coordinates escalation. It does not prove immunogenicity, activation, killing, or clinical utility.</div>
<div class="links">
...
<a class="pill" href="assets/cross_neo_md_audit/FLAGSHIP_COMMAND_CENTER.md">Markdown report</a>
<a class="pill" href="assets/cross_neo_md_audit/FLAGSHIP_COMMAND_CENTER_ONE_PAGE_KR.md">Korean brief</a>
</div></section>
<section><h2>Command Flow</h2><img src="assets/cross_neo_md_audit/fig_md73_flagship_command_center.png" alt="flagship command center"></section>
<section><h2>Command Board</h2>{table_html(board, ['lead','peptide','hla_4digit','physics_priority_score','preclinical_readiness','physics_tier','command_state','current_action','command_branch','next_wetlab_step','abort_condition','claim_boundary'], 10)}</section>
</main></body></html>"""
...
def deploy(paths: list[Path]) -> None:
    ...
    for fig in ["fig_md73_flagship_command_center"]:
        for ext in [".png", ".pdf"]:
            src = FIG / f"{fig}{ext}"
            if src.exists():
...
def main() -> None:
    ...
    board.to_csv(OUT / "flagship_command_center.tsv", sep="\t", index=False)
    make_figure(board)
    write_report(board, data)
    write_html(board, data)
    summary = {
        "n_flagships": int(len(board)),
        "top_priority": float(board["physics_priority_score"].max()) if not board.empty else 0.0,
        "top_readiness": float(board["preclinical_readiness"].max()) if not board.empty else 0.0,
        "boundary": "execution coordination only; not immunogenicity proof",
    }
    (OUT / "flagship_command_center_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    deploy(
        [
            OUT / "flagship_command_center.tsv",
            OUT / "FLAGSHIP_COMMAND_CENTER.md",
            OUT / "FLAGSHIP_COMMAND_CENTER_ONE_PAGE_KR.md",
        ]
    )
    print(json.dumps(summary, indent=2))
    print(f"[flagship-command-center] wrote {OUT}")
    print(f"[flagship-command-center] page {PAGE}")
