#!/usr/bin/env python3
"""v6 F12 — classical CPU baseline pipeline on the GSE193581 9-patient cohort.

Same pipeline as v6 Wave 1:
  QC -> normalize -> HVG -> PCA -> Harmony -> Leiden -> UMAP

Carries forward obs.braf_status / ras_status / braf_vs_ras / cancer_type.
"""
...
IN_H5AD = Path("/data/thca/scrna/raw/scrna_F12.h5ad")
OUT_H5AD = Path("/data/thca/scrna/processed/classical_baseline_F12.h5ad")
LOG = Path("/opt/thyroid-dash/project/logs/v6_F12_classical_baseline.log")
...
def main():
    ...
    if A.n_obs > target_total:
        ...
        print(f"  per-patient counts: {dict(A.obs['patient_id'].value_counts().sort_index())}", flush=True)
